...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "start_selection") initSnippingTool();
});
...
function initSnippingTool() {
    ...
    overlay.addEventListener('mouseup', (e) => {
        ...
        // Scale coordinates to match device pixel ratio (for retina displays)
        ...
        document.body.removeChild(overlay); // Remove overlay immediately

        if (finalRect.w > 10 && finalRect.h > 10) {
            // Send coordinates to background to capture screen
            chrome.runtime.sendMessage({ action: "capture_area", rect: finalRect });
        }
    });
}