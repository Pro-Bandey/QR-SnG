...
const qrMarginVal = document.getElementById("margin-val");
...
let currentCenterLogo = builtInLogos.qrsng; // Default brand logo
...
function updateThemeIcon() {
  ...
  if (isDark) {
    themeToggle.appendChild(createSafeSvgIcon('<svg height="24px" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" width="24px" fill="#e3e3e3"><path d="M480-120q-150 0-255-105T120-480q0-150 105-255t255-105q14 0 27.5 1t26.5 3q-41 29-65.5 75.5T444-660q0 90 63 153t153 63q55 0 101-24.5t75-65.5q2 13 3 26.5t1 27.5q0 150-105 255T480-120Zm0-80q88 0 158-48.5T740-375q-20 5-40 8t-40 3q-123 0-209.5-86.5T364-660q0-20 3-40t8-40q-78 32-126.5 102T200-480q0 116 82 198t198 82Zm-10-270Z"/></svg>'));
  } else {
    themeToggle.appendChild(createSafeSvgIcon('<svg height="24px" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M565-395q35-35 35-85t-35-85q-35-35-85-35t-85 35q-35 35-35 85t35 85q35 35 85 35t85-35Zm-226.5 56.5Q280-397 280-480t58.5-141.5Q397-680 480-680t141.5 58.5Q680-563 680-480t-58.5 141.5Q563-280 480-280t-141.5-58.5ZM200-440H40v-80h160v80Zm720 0H760v-80h160v80ZM440-760v-160h80v160h-80Zm0 720v-160h80v160h-80ZM256-650l-101-97 57-59 96 100-52 56Zm492 496-97-101 53-55 101 97-57 59Zm-98-550 97-101 59 57-100 96-56-52ZM154-212l101-97 55 53-97 101-59-57Zm326-268Z"/></svg>'));
  }
}
...
// Sync Margin text
qrMarginSlider.addEventListener('input', (e) => {
...
} [colorDarkInput, colorDark2Input, colorEyeInput, colorLightInput, transBgCheck].forEach((el) => {
  el.addEventListener("change", () => {
    checkContrast();
    generateQr(); // Force visual update in real-time
  });
});
...
async function generateQr() {
  ...
  const marginSize = parseInt(qrMarginSlider.value, 10);
  const requestedEcl = qrEclSelect.value;

  // Guardrail: Force High Error Correction if Logo exists to ensure scanning works.
  const finalEcl = currentCenterLogo ? "H" : requestedEcl;
  ...
  if (colorDarkInput.value !== colorDark2Input.value) {
    colorConfig = {
      color: undefined, // Clear solid color
      gradient: {
        type: "linear",
        rotation: Math.PI / 4,
        colorStops: [
  ...
  const eyeColorConfig = { color: colorEyeInput.value, gradient: null };
  ...
  if (!qrcodee.querySelector("canvas")) {
    qrcodee.textContent = '';
...
document.getElementById('qr-ecl').addEventListener('change', generateQr);
...
// Helper for building safe result trees
function createResultRow(container, labelText, valueText, isLink = false, linkHref = "") {
...
function parseScanResult(txt) {
  ...
  if (txt.startsWith("WIFI:")) {
    ...
    createResultRow(container, "Name", ssid);
    createResultRow(container, "Pass", pass);
    createResultRow(container, "Security", type);
    return container;
  }
  ...
  if (txt.startsWith("BEGIN:VCARD")) {
    ...
    createResultRow(container, "Name", n);
    createResultRow(container, "Phone", tel, true, `tel:${tel}`);
    createResultRow(container, "Email", em, true, `mailto:${em}`);
    return container;
  }

  const isLink = txt.startsWith("http://") || txt.startsWith("https://") || txt.startsWith("chrome:") || txt.startsWith("about:");

  if (txt.startsWith("http://wa.me") || txt.startsWith("https://wa.me") ||
    txt.startsWith("http://t.me") || txt.startsWith("https://t.me") || isLink) {

    actionbtn.style.display = "flex";

    if (txt.includes("wa.me")) {
      actionbtn.appendChild(createSafeSvgIcon(actionbtnSvg.sms));
      actionbtn.appendChild(document.createTextNode(" Send To WA"));
    } else if (txt.includes("t.me")) {
      actionbtn.appendChild(createSafeSvgIcon(actionbtnSvg.sms));
      actionbtn.appendChild(document.createTextNode(" Send To Tele"));
    } else {
  ...
  if (txt.startsWith("geo:") || txt.startsWith("mailto:") || txt.startsWith("smsto:")) {
    actionbtn.style.display = "flex";
    const strong = document.createElement("strong");
    ...
    if (txt.startsWith("geo:")) {
      ...
      strong.textContent = "Geo Location";
      ...
      strong.textContent = "Email Link";
    } else {
      actionbtn.appendChild(createSafeSvgIcon(actionbtnSvg.sms));
      actionbtn.appendChild(document.createTextNode(" Send SMS"));
      strong.textContent = "SMS Link";
    }
...
function scanQR(file) {
  ...
  iTag.textContent = "Scanning...";
  outqrtxt.appendChild(iTag);
    ...
    img.onload = () => {
      ...
      try {
        ...
        if (qrResult && qrResult.data) {
          ...
          outqrtxt.appendChild(err);
        }
      } catch (err) {
        outqrtxt.textContent = "";
        const errSpan = document.createElement("span");
        errSpan.style.color = "red";
        errSpan.textContent = "Scanning failed.";
...
function renderHistory() {
  ...
  if (filtered.length === 0) {
    ...
    emptyMsg.style.cssText = "text-align:center;font-size:12px;color:var(--text-muted);padding:20px;";
  ...
  extApi.storage.onChanged.addListener((changes, area) => {
    if (area === "local") {
      ...
      if (changes["qrAreaScan"] && changes["qrAreaScan"].newValue) {
        ...
        span.textContent = "Cropping & Scanning...";
        outqrtxt.appendChild(span);
        scanActions.style.display = 'none';
        ...
        img.onload = () => {
          ...
          try {
            ...
            if (qrResult && qrResult.data) {
              ...
              const err = document.createElement("span");
              err.style.color = "red";
              err.textContent = "No QR found in selected area.";
              outqrtxt.appendChild(err);
            }
          } catch (e) {
            outqrtxt.textContent = "";
            const err = document.createElement("span");
            err.style.color = "red";
            err.textContent = "Error scanning area.";
            outqrtxt.appendChild(err);
          }
